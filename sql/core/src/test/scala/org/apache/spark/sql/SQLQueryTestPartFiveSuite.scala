...
import org.apache.spark.tags.ExtendedSQLPartZeroTest
...
@ExtendedSQLPartZeroTest
class SQLQueryTestPartFiveSuite extends QueryTest with SharedSparkSession with SQLHelper
  ...
  protected def createScalaTestCase(testCase: TestCase): Unit = {
    if (ignoreList.exists(t =>
      ...
      case _ =>
        if (math.abs(testCase.name.hashCode) % 5 == 4) {
          test(testCase.name) {
            runTest(testCase)
          }
        }
