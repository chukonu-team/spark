...
class MathFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  @specialized(Int, Long, Float, Double) U](
    ...
    actualA.zip(expectedA).foreach { case (actual, expected) =>
      ...
      assert(approxEquals(actual.get(0), expected.get(0)), 
        s"Failed on a: actual ${actual.get(0)} did not approximately equal expected ${expected.get(0)}")
    }
    ...
    actualB.zip(expectedB).foreach { case (actual, expected) =>
      assert(actual.length == 1 && expected.length == 1)
      assert(approxEquals(actual.get(0), expected.get(0)), 
        s"Failed on b: actual ${actual.get(0)} did not approximately equal expected ${expected.get(0)}")
    }
  ...
  test("asinh") {
    testOneToOneMathFunctionApprox(asinh,
      (x: Double) => math.log(x + math.sqrt(x * x + 1)), 15)
  }
  ...
  test("atanh") {
    testOneToOneMathFunctionApprox(atanh,
      (x: Double) => (0.5 * (math.log1p(x) - math.log1p(-x))), 16)
  }
