...
class SparkEnv (
  ...
  def initializePythonDeamons(
      pythonExec: String,
      envVarsList: List[Map[String, String]]): Unit = {
    envVarsList.par.foreach { envVars =>
      val key = (pythonExec, envVars)
      val pythonWorker = synchronized {
        pythonWorkers.getOrElseUpdate(
          key, new PythonWorkerFactory(pythonExec, envVars)
        )
      }
      val (socket, _) = pythonWorker.create()
      pythonWorker.releaseWorker(socket)
    }
  }
}
