...
def wrap_cogrouped_map_pandas_udf(f, return_type, argspec):
    def wrapped(
        left_key_series, left_value_series, right_key_series, right_value_series
    ):
        import pandas as pd
    ...
    return lambda kl, vl, kr, vr: [
        (wrapped(kl, vl, kr, vr), to_arrow_type(return_type))
    ]
            ...
            if not (
                ...
                raise RuntimeError(
                    ...
                    "Expected: {} Actual: {}".format(
                        len(return_type), len(result.columns)
                    )
                )
...
def wrap_window_agg_pandas_udf(f, return_type, runner_conf, udf_index):
    ...
    window_bound_type = [t.strip().lower() for t in window_bound_types_str.split(",")][
        udf_index
    ]
...
def read_single_udf(pickleSer, infile, eval_type, runner_conf, udf_index):
    ...
    if eval_type == PythonEvalType.SQL_SCALAR_PANDAS_UDF:
        ...
        return arg_offsets, wrap_window_agg_pandas_udf(
            func, return_type, runner_conf, udf_index
        )
...
def read_udfs(pickleSer, infile, eval_type):
    ...
    if eval_type in (
        ...
        if eval_type == PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF_WITH_STATE:
            state_object_schema = StructType.fromJson(
                json.loads(utf8_deserializer.loads(infile))
            )
    ...
    if is_scalar_iter or is_map_pandas_iter or is_map_arrow_iter:
        ...
        def func(_, iterator):
            ...
            for result_batch, result_type in result_iter:
                ...
                assert (
                    is_map_pandas_iter
                    or is_map_arrow_iter
                    or num_output_rows <= num_input_rows
...
def main(infile, outfile):
    global last_stage_id
    global last_partition_id
    ...
    def append_event(event_name: str):
        trace_names.append(event_name)
        trace_timestamps.append(time.time())
    ...
    try:
        ...
        print_log("A", boot_time)
        append_event("ReceiveFirstMessage")
        ...
        if memory_limit_mb > 0 and has_resource_module:
            ...
            try:
                ...
                lineno = (
                    getframeinfo(currentframe()).lineno + 1
        ...
        print_log(
            "D",
            boot_time,
            stage_id=taskContext._stageId,
            partition_id=taskContext._partitionId,
        )
        ...
        if needs_broadcast_decryption_server:
            ...
            print_log("E0 (NeedsBroadcastDecryptionServer)", boot_time)
        ...
        append_event("ReceiveAndDeserializeUDFs")
    ...
    if read_int(infile) == SpecialLengths.END_OF_STREAM:
        ...
        append_event("FinalizeWorker")
        print_traces(
            boot_time,
            trace_names,
            trace_timestamps,
            taskContext._stageId,
            taskContext._partitionId,
        )
        last_stage_id = taskContext._stageId
        last_partition_id = taskContext._partitionId
    else:
