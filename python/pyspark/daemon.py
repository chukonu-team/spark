...
def worker(sock, authenticated):
    ...
    try:
        ...
        try:
            outfile.flush()
            t3 = time.time()
            stage = pyspark_worker.last_stage_id
            partition = pyspark_worker.last_partition_id
            print_trace("CallWorkerMain", t1, t2, stage, partition)
            print_trace("OutfileFlush", t2, t3, stage, partition)
        except Exception:
            pass
    return exit_code
...
def manager():
    ...
    daemon_end_time = time.time()
    print_trace("DaemonInitialization", daemon_begin_time, daemon_end_time, 0, 0)
